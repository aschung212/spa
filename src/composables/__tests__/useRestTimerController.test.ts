...
describe('useRestTimerController', () => {
  ...
  describe('pending "rest-again" intent (LIFT-751 / LIFT-1355)', () => {
    ...
    it('ignores a request that went stale waiting for a consumer', async () => {
      // e.g. a launch that landed on the auth gate. By the time the user signs in
      // and the tracker mounts, a rest between sets they never started is noise.
      requestRestAgain()
